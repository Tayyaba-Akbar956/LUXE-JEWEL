...
export default function NecklacesPage() {
    ...
    useEffect(() => {
        const fetchNecklaces = async () => {
            setLoading(true);
            const { data: catData } = await supabase.from('categories').select('id').eq('slug', 'necklaces').single();
            if (catData) {
            ...
            <main className="flex-grow">
                <section className="relative h-64 flex items-center justify-center overflow-hidden">
                    ...
                    <div className="relative text-center">
                        <h1 className="font-display text-5xl text-champagne-200">Necklaces Collection</h1>
                    ...
                    <div className="container mx-auto px-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            {loading ? [1, 2, 3, 4].map(i => <div key={i} className="aspect-[4/5] bg-luxury-dark/50 animate-pulse rounded-lg border border-gold-500/10" />)
                                : products.length === 0 ? <div className="col-span-full text-center py-20 text-silver-500">No necklaces found.</div>
